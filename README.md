...
Console keys: `↑/↓` select · `Alt+↑/↓` jump between heads (works while focused) · `Enter` focus terminal · `Ctrl+Q` detach · `s` save/unsave a head's path (★) · `Ctrl+N` new head · `R` rename · `Ctrl+X` close · **drag mouse to select→copy** · **`/` search history** (`n`/`N` cycle) · mouse-wheel / `Shift+PgUp`/`PgDn` scroll · `F1` help · `F5` refresh · `q` `q` quit (twice). The new-head prompt supports **`Tab` path completion**, **`↑/↓` to pick a saved path**, and **`Ctrl+T`** to switch Claude / plain shell / custom command (`ssh host`). In a focused terminal all keys pass through to Claude/the shell — `Ctrl+C`, `Ctrl+←/→` (word motion), `Ctrl+Backspace` (delete word), tab-completion, `Ctrl+R` shell reverse-search, and paste. Copy uses OSC 52 (iTerm2 / SSH-friendly) plus the native clipboard tool. Runs on Linux and macOS; native Windows builds via ConPTY (untested) and works today under WSL. See [`ROADMAP.md`](ROADMAP.md) — the top remaining item is head persistence across quit.
